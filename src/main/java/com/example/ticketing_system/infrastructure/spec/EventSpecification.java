...
public class EventSpecification {
    public static Specification<EventEntity> nameLike(String name) {
        // Ya no validamos null aquí
        return (root, query, cb) ->
                cb.like(cb.lower(root.get("eventName")), "%" + name.toLowerCase() + "%");
    }

    public static Specification<EventEntity> dateFrom(LocalDateTime start) {
        return (root, query, cb) ->
                cb.greaterThanOrEqualTo(root.get("eventDate"), start);
    }

    public static Specification<EventEntity> dateTo(LocalDateTime end) {
        return (root, query, cb) ->
